#!/usr/bin/env python3
"""ANGLE-V2 release manifest (adapted from p10-release-manifest.py). Marks v2 as superseding-pending-owner-review; v1 media untouched.
...
Usage (repo root, after scripts/angle-v2-encode.sh):
    python3 scripts/angle-v2-release-manifest.py
Outputs:
    docs/benchmarks/captures/angle-v2-data/angle-v2-release-manifest.json
    docs/benchmarks/captures/angle-v2-data/angle-v2-release-notes.md
"""
import hashlib
import json
import pathlib
...
EVIDENCE = ROOT / "docs/benchmarks/captures/angle-v2-data"
...
def main() -> None:
    ...
    freeze = json.loads((OUT / "angle-v2-freeze.json").read_text())
    duration = T1 - T0 + PAD
    ...
    manifest = {
        "release": "soldier-view-media-v2-PENDING-OWNER-REVIEW", "supersedes": "soldier-view-media-v2 (superseding-pending-owner-review: the shipped v1 media stays untouched on disk and in the release until the owner adopts v2)",
        "viewpointId": VP,
        ...
        "media": media,
        "publishCommand": (
            "gh release create soldier-view-media-v2 "
            f"app/RenderOutput/angle-v2/{VP}.full.mp4 "
            f"app/RenderOutput/angle-v2/{VP}.proxy.mp4 "
            "docs/benchmarks/captures/angle-v2-data/angle-v2-release-manifest.json "
            "--title 'Soldier View media v2 (PENDING OWNER REVIEW) — garnett-road-to-angle' "
            "--notes-file docs/benchmarks/captures/angle-v2-data/angle-v2-release-notes.md"
        ),
        "note": "Owner publishes at Phase 12 (plan §12). Do not publish "
                "from Phase 10.",
    }
    EVIDENCE.mkdir(parents=True, exist_ok=True)
    (EVIDENCE / "angle-v2-release-manifest.json").write_text(
        json.dumps(manifest, indent=2) + "\n")

    notes = f"""# Soldier View media v2 (PENDING OWNER REVIEW) — With Garnett's Line
...
"""
    (EVIDENCE / "angle-v2-release-notes.md").write_text(notes)
