#!/bin/sh
# Iverson rolling chunk harvester (the P10 pattern).
#
# The full 2560x1440 PNG sequence (~36,315 frames x ~3.2 MB ≈ 116 GB)
# does not fit this machine's free disk. Run this LOOP alongside
# IversonProductionRender.RenderProduction: whenever a chunk's manifest
# appears (written only after every frame of the chunk is on disk), it
...
#      so chunk mp4s concat losslessly into the exact stream a
#      single-pass encode of the same frames would give),
#   3. verifies the encoded frame count by decoding the chunk,
#   4. deletes the chunk's PNGs (regenerable: the render is resumable
#      and deterministic; ChunkComplete accepts an encoded chunk).
#
# Steady-state disk use stays at ~2 chunks of PNGs (~12 GB).
#
# Usage: scripts/iverson-chunk-harvester.sh [--once]
#   loops until app/RenderOutput/iverson/render-done exists and all
#   chunks are harvested; --once does a single pass.
set -eu
