#!/bin/sh
# Iverson production encode (the P10 pattern; media contract
# docs/reconstruction/p1-media-contract.md).
#
# Two source modes:
#   A. seq-full holds the complete PNG sequence -> single-pass encode.
#   B. the rolling harvester (scripts/iverson-chunk-harvester.sh)
#      already encoded per-chunk mp4s with the FINAL codec settings and
...
#
# Deliverables:
#   iverson-forney-field.full.mp4   2560x1440p30 H.264 CRF18,
#                                   1 keyframe/s, +faststart, AAC 192k
#                                   deterministic full mix
#   iverson-forney-field.proxy.mp4  1280x720p30 CRF20, same audio
#   iverson-media.sha256            checksums + measured sizes/bitrates
#
# Usage: scripts/iverson-encode.sh   (repo root, after RenderProduction
#   [+ harvester] and the stem build)
set -eu
...
[ -f "$STEMS/mix.wav" ] || {
  ...
  echo "    --events ../docs/benchmarks/captures/iverson-production/iverson-audio-events.json \\" >&2
  echo "    --out ../app/RenderOutput/iverson/stems-full --t0 $T0 --t1 $(python3 -c "print($T1+$PAD)")" >&2
  exit 1
}
